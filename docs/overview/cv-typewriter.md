...
CV Typewriter is a single-page React application that lets users craft professional CVs using a data-first, JSON-driven approach. The UI is split into an editor panel (live JSON editing via CodeMirror) and a paginated A4 preview panel. The application normalizes and renders bilingual CV data (Spanish/English) with an Oxford-inspired typographic design, supports Markdown-style links in descriptions, and exports clean PDF output via the browser's print-to-PDF feature.
...
| `src/features/cv-typewriter/hooks/useCVData.jsx`         | Manages JSON text state, file name, active preview language, safe parsing, available languages, and normalized CV object.                     |
...
| `src/features/cv-typewriter/components/EditorPanel.jsx`  | Left-side panel with file name input, preview language selector, quick action buttons, and the CodeMirror JSON editor.                        |
...
| `src/features/cv-typewriter/cvUtils.jsx`                 | Utility functions for safe JSON parsing, language selection, date formatting, Markdown link rendering, unit conversion, and CV normalization. |
| `src/features/cv-typewriter/sample.js`                   | Default bilingual sample data using `cv.languages.es` and `cv.languages.en`, plus shared design settings.                                     |
